...
if(isset($_POST['enter']))
 {
  ...
  {
$time = time();
    $query = mysqli_query($connection,"INSERT INTO admin_login VALUES('','$adminmail','$date','$time')");
...
<div class="flex items-center justify-center h-screen">
  ...
  <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" action="admin_login.php" method="POST">
    <h4>Admin Login</h4><br>
    <div class="mb-4">
      <label class="block text-gray-700 text-sm font-bold mb-2" for="username">
        EMAIL
      </label>
      <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="username" type="email" placeholder="enter mail" name="email" required>
    </div>
     <?php   
    ...
    <div class="mb-6">
      ...
      <input class="shadow appearance-none border border-red-500 rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline" id="password" name="key" type="password" placeholder="******************" required>
      <?php
      if(in_array("YOUR KEY DIDN'T MATCH!",$error))
      {
        echo '<span style="color:red;">YOUR KEY DIDNOT MATCH </span>';
...
<!--       <p class="text-red-500 text-xs italic">Please choose a password.</p>
 -->    </div>
    <div class="flex items-center justify-between">
      <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" type="submit" name="enter">
      Log In
      </button>
    </div><br>
     <?php
      ...
      {
        echo '<span style="color:green;">SUCCESFULLY LOGGED IN</span>';

...
$pass2=$_POST['password2'];
  ...
  {
    $checkdatabase = mysqli_query($connection,"SELECT * FROM signin WHERE email = '$email'");
    $row=mysqli_num_rows($checkdatabase);
  }
  else{
  ...
  if($row == 1)
    {
      ...
      array_push($err,"YOUR EMAIL DOESNOT EXIST!");
    }

    if(preg_match("/(.*[a-zA-Z]){3}/i",$pass) == TRUE)
     {

     }
     else{
            array_push($err,"YOUR PASSWORD IS NOT OF CORRECT FORM!");
                            } 
    if(preg_match("/(.*[0-9]){3}/i",$pass) == TRUE)
     {
...
if(empty($err))
{
  unset($_SESSION['emailrec']);
   $pass = md5($pass);
   $query = mysqli_query($connection,"UPDATE signin SET password = '$pass' WHERE email = '$email'");  
   array_push($err,"YOUR PASSWORD IS CHANGED!YOU CAN LOGIN NOW!");
  ...
  if(isset($_POST['sub']))
{
  ...
  $email=$_POST['email'];
  $_SESSION['emailrec'] = $email;
  array_push($err,"SORRY CANT CHANGE YOUR PASSWORD!");
}
}
...
echo' <li><a href="#" data-toggle="modal" data-target="#employee-reg-modal" >
 ...
 echo 
      ...
      ' <a href="#" data-toggle="modal" data-target="#admin-panel-modal">
                          ADMIN PANEL
            </a>';}?> 
...
<div class="flex items-center justify-center h-screen">
  ...
  <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" action="password-recovery.php" method="POST">
    
    <h1>RECOVERY FORM</h1>
    <div class="mb-4">
     <label class="block text-gray-700 text-sm font-bold mb-2"> email </label>
     <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="email" name="email" placeholder="enter  your email" required value="<?php if(isset($_SESSION['emailrec'])){
      echo $_SESSION['emailrec'];
     } ?>"><br>
     <?php if(in_array("ENTER VALID EMAIL!",$err))
     {
      echo '<span style = "color :red;">ENTER VALID EMAIL PLEASE!</span>';
     }
        if(in_array("YOUR EMAIL DOESNOT EXIST!",$err))
        {
                echo '<span style = "color :red;">YOUR EMAIL DOESNOT EXIST!</span>';
  ...
  <div class="mb-4">
     ...
     <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="password" class="pass" name="password" placeholder="Enter your new password" required><br>
     <?php
      if(in_array("YOUR PASSWORD IS NOT OF CORRECT LENGTH!",$err))
      {
      echo '<span style = "color :red;">PASSWORD IS NOT OF CORRECT LENGTH!</span>';

      } 
     if(in_array("YOUR PASSWORD IS NOT OF CORRECT FORM!",$err))
      {
            echo '<span style = "color :red;">YOUR PASSWORD IS NOT OF CORRECT FORM!</span>';
    ...
    <div class="mb-4">
     ...
     <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="password" class="pass" name="password2" placeholder="Re-enter" required><br>
     <?php 
           if(in_array("YOUR PASSWORD IS NOT SAME!",$err))
           {
                        echo '<span style = "color :red;">YOUR PASSWORD IS NOT SAME!</span>';
     ...
     <?php
               if(in_array("YOUR PASSWORD IS CHANGED!YOU CAN LOGIN NOW!",$err))
               {
                echo '<span style = "color :green;">    YOUR PASSWORD IS CHANGED!</span>';

               } ?><br><br>
     <a href="login.php">BACK TO LOGINPAGE!</a>
   </div>
</form> 
</div>
</div>
